...
import { Action } from "@/types/business";
import {
...
const ActionPlan = () => {
  ...
  const toggleAction = async (id: string) => {
    ...
    const newStatus: Action["status"] = isNowCompleted
      ? "completed"
      : "pending";

    const success = await updateActionStatus(id, newStatus);
    if (success) {
      setActions((prev) =>
        prev.map((a) =>
          a.id === id
    ...
    <div className="p-4 sm:p-6 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 border-b pb-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <Button
            size="lg"
            className="gap-2 font-bold shadow-md bg-primary hover:bg-primary/90"
          >
              ...
              <CardHeader className="pb-6 border-b bg-muted/20">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-2">
                    <Badge
                      variant="outline"
                      className="bg-primary/5 text-primary border-primary/20 text-[10px] uppercase font-black tracking-widest px-2 py-0.5"
                    >
                        ...
                        <div className="flex items-center gap-3">
                          <h4
                            className={`font-bold text-lg leading-none tracking-tight ${
                              action.status === "completed"
                                ? "line-through text-muted-foreground decoration-2"
                                : ""
                            }`}
                          >
                            {action.title}
        ...
        <div className="space-y-6">
          <Card className="bg-primary shadow-2xl border-none text-primary-foreground overflow-hidden relative">
            ...
            <CardContent className="space-y-8 relative">
              ...
              <div className="grid grid-cols-1 gap-4 pt-2">
                <div className="p-4 bg-white/10 rounded-2xl border border-white/10 backdrop-blur-sm">
                  <p className="text-[10px] font-black uppercase tracking-[0.2em] text-white/60 mb-1">
                    {t("action_plan.impact_card.roi")}
                  </p>
                  <p className="text-3xl font-black">
                    {t("action_plan.impact_card.roi_value")}
                  </p>
                </div>

                <div className="p-4 bg-white/10 rounded-2xl border border-white/10 backdrop-blur-sm">
                  <p className="text-[10px] font-black uppercase tracking-[0.2em] text-white/60 mb-1">
                    {t("action_plan.impact_card.time_saved")}
                  </p>
                  <p className="text-3xl font-black">
                    {t("action_plan.impact_card.time_saved_value")}{" "}
                    <span className="text-sm font-normal opacity-60">
                      {t("action_plan.impact_card.time_saved_unit")}
                    </span>
                  </p>
                </div>
              </div>
              ...
              <Button
                className="w-full h-12 gap-2 bg-white text-primary hover:bg-white/90 font-black shadow-lg"
                variant="secondary"
              >
              ...
              <div className="space-y-4">
                ...
                <div className="p-4 bg-background/50 rounded-xl border border-dashed text-center">
                  <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">
                    {t("action_plan.next_target.waiting")}
                  </span>
                </div>
              </div>
            </CardContent>
