...
import { RadarItem, Domain, PriorityLevel } from '@/types/business';
...
const MOCK_RADAR_ITEMS: RadarItem[] = [
  ...
  },
  {
    ...
    diagnosis: {
      ...
      cause: 'Burn rate mensal de R$ 45K com reserva de apenas R$ 158K. Receita não cobre custos operacionais desde outubro.',
      effect: 'Risco de insolvência. Necessidade de rodada de investimento emergencial ou corte drástico de custos.',
    ...
    recommendation: {
      ...
      description: 'Reduzir imediatamente custos fixos em 30% (escritório, softwares não críticos). Negociar recebíveis e antecipar faturas.',
  ...
  },
  {
    ...
    diagnosis: {
      ...
      cause: 'Dados de aquisição mostram que clientes SaaS têm CAC 30% menor e LTV 40% maior que outros segmentos.',
      ...
      dataSources: ['Google Analytics', 'CRM', 'Planilha de Custos de Aquisição']
    },
    recommendation: {
      id: 'rec4',
      title: 'Campanha Focada em SaaS + Parcerias',
      description: 'Criar campanha de marketing específica para SaaS. Desenvolver case studies e conteúdo técnico.',
      expectedImpact: '+40% de novos clientes SaaS',
      timeframe: '90 dias',
      complexity: 'low',
      confidence: 72,
      levers: [
        { type: 'volume', label: 'Volume', impact: 'high' }
      ]
    },
...
const Radar: React.FC = () => {
  ...
  const [selectedDomains, setSelectedDomains] = useState<Domain[]>(['finance', 'commercial', 'operations', 'strategy', 'marketing']);
  ...
  const filteredItems = useMemo(() => {
    return items.filter(item => {
      ...
      const scoreA = priorityWeight[a.priority] * 10 + urgencyWeight[a.urgency];
      const scoreB = priorityWeight[b.priority] * 10 + urgencyWeight[b.urgency];
  ...
  // Estatísticas
  const stats = useMemo(() => {
    const risks = items.filter(i => i.type === 'risk' && selectedDomains.includes(i.domain));
    const opportunities = items.filter(i => i.type === 'opportunity' && selectedDomains.includes(i.domain));
    const highPriority = items.filter(i => i.priority === 'high' && selectedDomains.includes(i.domain));
    const critical = risks.filter(r => (r.alert as any).severity === 'critical');
        ...
        <div className="flex flex-wrap gap-3">
          <Card className="bg-red-50 border-red-200">
            <CardContent className="p-3 flex items-center gap-2">
              ...
              <div>
                <div className="text-lg font-bold text-red-700">{stats.risks}</div>
          ...
          <Card className="bg-emerald-50 border-emerald-200">
            <CardContent className="p-3 flex items-center gap-2">
              ...
              <div>
                <div className="text-lg font-bold text-emerald-700">{stats.opportunities}</div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="w-full sm:w-auto grid grid-cols-2 sm:grid-cols-5 gap-1">
            ...
            <TabsTrigger value="risks" className="gap-2">
              ...
              <Badge variant="secondary" className="ml-1 bg-red-100 text-red-700">{stats.risks}</Badge>
            </TabsTrigger>
            <TabsTrigger value="opportunities" className="gap-2">
              <TrendingUp className="h-4 w-4" />
              <span className="hidden sm:inline">Oportunidades</span>
              <Badge variant="secondary" className="ml-1 bg-emerald-100 text-emerald-700">{stats.opportunities}</Badge>
            </TabsTrigger>
            <TabsTrigger value="high" className="gap-2">
              <Target className="h-4 w-4" />
              <span className="hidden sm:inline">Alta Prioridade</span>
              <Badge variant="secondary" className="ml-1 bg-amber-100 text-amber-700">{stats.highPriority}</Badge>
            </TabsTrigger>
          </TabsList>
              ...
              <Card className="border-dashed">
                <CardContent className="p-12 text-center">
                  ...
                  <p className="text-muted-foreground max-w-md mx-auto">
                    ...
                    Ajuste os filtros de domínio ou aguarde novas análises da IA.
                  </p>
                </CardContent>
              </Card>
            ) : (
      ...
      <div className="pt-6 border-t">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 text-sm text-muted-foreground">
          <div className="flex items-center gap-2">
            ...
            <span>
              Última análise: <strong>Hoje às 08:30</strong> | 
              Próxima: <strong>Amanhã às 06:00</strong>
            </span>
          </div>
