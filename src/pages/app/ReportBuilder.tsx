...
import { getTemplatesFromSupabase } from "@/services/supabaseReportsService";
  ...
  const handleNext = async () => {
    if (step === 1) {
      ...
      if (
        ...
        (!urlData.trim() || !urlData.startsWith("http"))
      ) {
        toast({
          title: t("builder.toasts.data_required_title"),
  ...
  const handleCreateReport = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: error?.message || t("builder.errors.save_generic"),
        variant: "destructive",
      });
    } finally {
      setLoading(false);
        ...
        <div className="container mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 text-sm">
            <span className="hidden sm:inline text-muted-foreground">
              {t("builder.step_indicator.prefix")}
            </span>
            <span className="font-medium">
              {t("builder.step_indicator.value", { current: step, total: 5 })}
            </span>
          </div>
        </div>
            ...
            <Card className="border-none shadow-none bg-transparent">
              <CardHeader className="px-2">
                <CardTitle className="text-lg">
                  {t("builder.sidebar.summary_title")}
                </CardTitle>
              </CardHeader>
              ...
              {step === 1 && (
                <div className="bg-primary/5 border border-primary/20 rounded-xl p-4 flex items-start gap-3 mb-6">
                  ...
                  <p className="text-sm text-primary/90">
                    <strong>{t("builder.sidebar.consultant_hint")}:</strong>{" "}
