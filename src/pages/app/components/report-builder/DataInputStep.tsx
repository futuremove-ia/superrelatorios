    ...
    if (uploadedFile) {
      ...
      try {
        ...
        toast({
          ...
          description: t("builder.toasts.file_uploaded_desc", {
    ...
    <div className="space-y-6">
      <div className="text-center">
        <h2 className="text-xl md:text-2xl font-semibold mb-2">
          {t("builder.steps.data.title")}
        </h2>
        <p className="text-muted-foreground">{t("builder.steps.data.desc")}</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card
          ...
          onClick={() =>
            setFormData((prev) => ({ ...prev, dataSource: "upload" }))
          }
        >
          <CardHeader className="text-center">
            <Upload className="w-8 h-8 mx-auto mb-2 text-primary" />
            <CardTitle className="text-lg">
              {t("builder.steps.data.sources.upload")}
            </CardTitle>
            <CardDescription>
        ...
        <Card
          ...
          onClick={() =>
            setFormData((prev) => ({ ...prev, dataSource: "text" }))
          }
        >
          <CardHeader className="text-center">
            <Copy className="w-8 h-8 mx-auto mb-2 text-primary" />
            <CardTitle className="text-lg">
              {t("builder.steps.data.sources.paste")}
            </CardTitle>
            <CardDescription>
        ...
        <Card
          ...
          onClick={() =>
            setFormData((prev) => ({ ...prev, dataSource: "url" }))
          }
        >
          <CardHeader className="text-center">
            <Globe className="w-8 h-8 mx-auto mb-2 text-primary" />
            <CardTitle className="text-lg">
              {t("builder.steps.data.sources.url")}
            </CardTitle>
            <CardDescription>
      ...
      {formData.dataSource === "upload" && (
        <Card>
          <CardContent className="p-6">
            <div className="border-2 border-dashed border-muted rounded-xl p-8 text-center">
              ...
              <div className="space-y-2">
                ...
                <button
                  ...
                  onClick={() =>
                    toast({
                      title: t("builder.toasts.model_downloaded_title"),
                      description: t("builder.toasts.model_downloaded_desc"),
                    })
                  }
            ...
            {/* First Win - Auto KPI Analysis */}
            {parsedData && parsedData.headers.length > 0 && (
              <div className="mt-6">
                <FirstWinCard
                  parsedData={{
                    headers: parsedData.headers,
                    rows: parsedData.rows as Record<string, unknown>[],
                  }}
                />
              </div>
            )}
          ...
          <CardContent className="p-6">
            <div className="space-y-2">
              <Label htmlFor="textData">
                {t("builder.steps.data.labels.paste_area")}
              </Label>
              <Textarea
                id="textData"
                value={textData}
      ...
      {formData.dataSource === "url" && (
        <Card>
          <CardContent className="p-6">
            <div className="space-y-2">
              <Label htmlFor="urlData">
                {t("builder.steps.data.labels.url_input")}
              </Label>
              ...
              <p className="text-xs text-muted-foreground">
                Dica: Certifique-se de que o link é público para que nossa IA
