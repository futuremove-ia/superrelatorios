...
const MetricsDashboard = () => {
  ...
  const getDomainName = (domain: string) => {
    switch (domain) {
      ...
      case 'operations': return 'Operacional';
      case 'strategy': return 'Estratégico';
    ...
    <div className="container-fluid space-y-8 py-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-bold text-foreground">Painel de Indicadores</h1>
            <p className="text-muted-foreground">Monitoramento completo de métricas por domínios</p>
          </div>
        </div>
      ...
      {mockDomainData.some(d => d.criticalCount > 0) && (
        <Alert className="border-yellow-200 bg-yellow-50">
          ...
          <AlertDescription className="text-yellow-800">
            <strong>Atenção:</strong> {mockDomainData.filter(d => d.criticalCount > 0).length} domínios possuem métricas críticas que requerem atenção imediata.
            <Button variant="link" size="sm" className="p-0 h-auto ml-2 text-yellow-600">
              Ver detalhes
            </Button>
          </AlertDescription>
        </Alert>
              ...
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-lg font-bold">{domain.healthScore}/100</span>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-2 text-sm">
                  ...
                  <div>
                    ...
                    <p className="font-medium text-red-600">{domain.criticalCount}</p>
                  </div>
                </div>
                
                <div className="flex items-center justify-between">
                  <TrendIndicator trend={domain.trend > 0 ? 'up' : 'down'} value={Math.abs(domain.trend)} />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredMetrics.map((metric) => (
              <MetricsCard
                key={metric.id}
                metric={metric}
                onClick={() => {/* Handle metric detail view */}}
              />
            ))}
          </div>
        )}
                  ...
                  <tbody>
                    {filteredMetrics.map((metric) => (
                      <tr key={metric.id} className="border-b">
                        ...
                        <td className="p-2">
                          <Badge className={getHealthColor(metric.healthStatus)}>
                            {metric.healthStatus}
                          </Badge>
                        </td>
                        <td className="p-2">
