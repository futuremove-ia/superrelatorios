...
import { mockPriorities as initialMockPriorities } from '@/services/mockBusiness';
      ...
      <div className="flex items-center gap-4 flex-1 min-w-0">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 mb-1">
            <h4 className="font-bold text-sm text-foreground truncate">{priority.title}</h4>
            <Badge variant="outline" className={cn("text-[10px] h-4 px-1", getLevelColor(priority.level))}>
              {t(`priorities.levels.${priority.level}`)}
            </Badge>
          </div>
          <p className="text-xs text-muted-foreground line-clamp-1">{priority.explanation}</p>
        </div>
      </div>
      
      <div className="flex items-center gap-4 w-full sm:w-auto justify-between sm:justify-end">
        <div className="flex flex-col items-end">
          <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">{t('priorities.card.score')}</span>
          <span className="text-sm font-black text-foreground">{priority.score.calculatedValue}</span>
        </div>
        ...
        <Button 
          ...
          onClick={() => priority.status === 'suggested' ? onValidate(priority) : onAction(priority.id)}
        >
...
const Priorities = () => {
  ...
  const [priorities, setPriorities] = useState<Priority[]>(initialMockPriorities);
  ...
  const handleValidatePriority = (id: string, finalScore: PriorityScore) => {
    setPriorities(prev => prev.map(p => 
      p.id === id 
        ? { ...p, status: 'validated', score: finalScore } 
        : p
    ));
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between mb-2">
            ...
            <Badge variant="secondary" className="font-bold">
              {filteredPriorities.length} {t('priorities.items_count', { count: filteredPriorities.length })}
            </Badge>
          </div>
