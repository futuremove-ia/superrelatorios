...
export class RiskService {
  ...
  async createMitigation(
    ...
    if (!parsed.success) {
      throw new Error(
        `Invalid mitigation data returned: ${parsed.error.message}`,
      );
    }
  ...
  async generateRiskMatrix(organizationId: string): Promise<RiskMatrix> {
        ...
        Array(10)
          ...
          .map((_, j) => {
            ...
            if (cellRisks.length > 0) {
              const avgScore =
                cellRisks.reduce((sum, r) => sum + r.risk_score, 0) /
                cellRisks.length;
    ...
    const summary = {
      ...
      mediumRisk: risks.filter((r) => r.risk_score >= 30 && r.risk_score < 70)
        .length,
   ...
   * Get risk alerts
   */
  async getRiskAlerts(organizationId: string): Promise<
    Array<{
      risk: Risk;
  ...
  async getRiskDashboard(organizationId: string): Promise<{
    ...
    const recentRisks = matrix.risks.filter(
    ...
    const upcomingDeadlines = allMitigations
      ...
      .map((m) => {
        ...
        const daysUntilDeadline = m.deadline
          ? Math.ceil(
              (new Date(m.deadline).getTime() - new Date().getTime()) /
                (1000 * 60 * 60 * 24),
            )
          : 0;
