-- =====================================================
-- METRICS LIBRARY UNIFICATION v1.0 - FIXED v2
-- Based on Cascade.app patterns
-- Fixed: DATE_TRUNC type casting + GROUP BY issues
-- =====================================================
...
-- View: Metric values by period with aggregations - FIXED VERSION v2
...
WITH grouped_metrics AS (
    SELECT 
        ...
        mv.organization_id,
        o.name as organization_name,
        -- FIXED: Convert enum to text for DATE_TRUNC
        DATE_TRUNC(ml.group_data_period::text, mv.period_start)::date as period_start,
        DATE_TRUNC(ml.group_data_period::text, mv.period_start)::date + 
            CASE 
                WHEN ml.group_data_period::text = 'daily' THEN INTERVAL '1 day' - INTERVAL '1 second'
                WHEN ml.group_data_period::text = 'weekly' THEN INTERVAL '1 week' - INTERVAL '1 second'
                WHEN ml.group_data_period::text = 'monthly' THEN INTERVAL '1 month' - INTERVAL '1 second'
                WHEN ml.group_data_period::text = 'quarterly' THEN INTERVAL '3 months' - INTERVAL '1 second'
                WHEN ml.group_data_period::text = 'half_yearly' THEN INTERVAL '6 months' - INTERVAL '1 second'
                WHEN ml.group_data_period::text = 'yearly' THEN INTERVAL '1 year' - INTERVAL '1 second'
            END as period_end,
        ...
        ARRAY_AGG(mv.value ORDER BY mv.created_at DESC) as all_values
    FROM metric_values mv
    JOIN metrics_library ml ON mv.metric_id = ml.id
    JOIN organizations o ON mv.organization_id = o.id
    WHERE ml.is_active = true
    GROUP BY 
        ...
        mv.organization_id, o.name, DATE_TRUNC(ml.group_data_period::text, mv.period_start)
)
SELECT 
    metric_id,
    metric_name,
    ...
    period_end,
    CASE 
        WHEN total_is = 'sum_of_values' THEN sum_value
        WHEN total_is = 'average_of_values' THEN avg_value
        WHEN total_is = 'last_value' THEN (all_values[1]) -- First element is the latest
        ELSE sum_value
