...
export const getCanonicalRoute = (path: string): string => {
  for (const [lang, routes] of Object.entries(routeMap)) {
    for (const [canonical, localized] of Object.entries(routes)) {
      ...
      if (
        (localized as string).includes(":") &&
        path.startsWith((localized as string).split(":")[0])
      ) {
        const paramValue = path.replace(
          (localized as string).split(":")[0],
          "",
        );
...
export const detectLanguageFromRoute = (path: string): string => {
  for (const [lang, routes] of Object.entries(routeMap)) {
    for (const route of Object.values(routes)) {
      ...
      if (
        path === routeStr ||
        (routeStr.includes(":") && path.startsWith(routeStr.split(":")[0]))
      ) {
        return lang;
      }
    }
