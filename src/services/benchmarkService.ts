...
export class BenchmarkService {
   ...
   */
  async getOrganizationBenchmarks(organizationId: string): Promise<OrganizationBenchmark[]> {
    ...
    if (error) {
      ...
      throw new Error(`Failed to fetch organization benchmarks: ${error.message}`)
    }
  ...
  async getOrganizationBenchmarkByKPI(
    ...
    if (error) {
      ...
      throw new Error(`Failed to fetch organization benchmark by KPI: ${error.message}`)
    }
   ...
   */
  async createOrganizationBenchmark(benchmark: BenchmarkCreate): Promise<OrganizationBenchmark> {
    const { data, error } = await this.supabase
      .from('organization_benchmarks')
      .insert(benchmark)
      .select()
    ...
    if (error) {
      ...
      throw new Error(`Failed to create organization benchmark: ${error.message}`)
    }

    const parsed = organizationBenchmarkSchema.safeParse(data)
    if (!parsed.success) {
      throw new Error(`Invalid organization benchmark data returned: ${parsed.error.message}`)
    }
  ...
  async updateOrganizationBenchmark(
    ...
    if (error) {
      ...
      throw new Error(`Failed to update organization benchmark: ${error.message}`)
    }
  ...
  async deleteOrganizationBenchmark(id: string): Promise<void> {
    ...
    if (error) {
      ...
      throw new Error(`Failed to delete organization benchmark: ${error.message}`)
    }
  }
  ...
  async getMarketBenchmarksByContext(
    ...
    if (error) {
      ...
      throw new Error(`Failed to fetch market benchmarks by context: ${error.message}`)
    }
   ...
   */
  async createMarketBenchmark(benchmark: MarketBenchmarkCreate): Promise<MarketBenchmark> {
    const { data, error } = await this.supabase
      .from('market_benchmarks')
      .insert(benchmark)
      .select()
    ...
    if (!parsed.success) {
      throw new Error(`Invalid market benchmark data returned: ${parsed.error.message}`)
    }
  ...
  async getBestBenchmark(
    ...
    try {
      ...
      const internalBenchmark = await this.getOrganizationBenchmarkByKPI(organizationId, kpiId)
      ...
      const marketBenchmarks = await this.getMarketBenchmarksByContext(kpiId, context)
      ...
      if (marketBenchmarks.length > 0) {
        ...
        if (context.company_size && bestMarketBenchmark.company_size === context.company_size) {
          relevanceScore += 30
        }
        
        if (context.industry_sector && bestMarketBenchmark.industry_sector === context.industry_sector) {
          relevanceScore += 30
        }
  ...
  async calculateBenchmarkLevels(targetValue: number): Promise<{
    ...
    const { data, error } = await this.supabase.rpc('calculate_benchmark_levels', {
  ...
  async getBenchmarkAnalysis(
    ...
    performance: 'excellent' | 'good' | 'average' | 'warning' | 'critical' | 'no_data'
    ...
    for (const kpiId of kpiIds) {
      try {
        ...
        if (actualValue && benchmark) {
          ...
          if (percentage >= (benchmark.value_excellent || benchmark.value_target * 1.3)) {
            performance = 'excellent'
          } else if (percentage >= (benchmark.value_good || benchmark.value_target * 1.1)) {
            performance = 'good'
  ...
  async getIndustrySectors(): Promise<string[]> {
    ...
    const sectors = [...new Set((data || []).map((item: any) => item.industry_sector))]
  ...
  async getCompanySizes(): Promise<string[]> {
    ...
    const sizes = [...new Set((data || []).map((item: any) => item.company_size))]
