...
export const RISK_KEYS = {
  ...
  byStatus: (orgId: string, status: string) =>
    ["risks", orgId, "status", status] as const,
  byId: (id: string) => ["risks", id] as const,
  mitigations: (riskId: string) => ["risks", "mitigations", riskId] as const,
  allMitigations: (orgId: string) =>
    ["risks", "mitigations", "orgId", orgId] as const,
  ...
  return useMutation({
    ...
    onSuccess: (newRisk, variables) => {
      ...
      queryClient.invalidateQueries({
        queryKey: RISK_KEYS.all(variables.organization_id),
      });
      queryClient.invalidateQueries({
        queryKey: RISK_KEYS.matrix(variables.organization_id),
      });
      queryClient.invalidateQueries({
        queryKey: RISK_KEYS.dashboard(variables.organization_id),
      });
...
export function useUpdateRisk() {
  ...
  return useMutation({
    mutationFn: ({
      ...
      id: string;
      updates: Partial<RiskCreate>;
    }) => riskService.updateRisk(id, updates),
    onSuccess: (updatedRisk, variables) => {
      // Invalidar queries relevantes
      queryClient.invalidateQueries({
        queryKey: RISK_KEYS.all(variables.updates.organization_id),
      });
      queryClient.invalidateQueries({
        queryKey: RISK_KEYS.byId(updatedRisk.id),
      });
      queryClient.invalidateQueries({
        queryKey: RISK_KEYS.matrix(variables.updates.organization_id),
      });
      queryClient.invalidateQueries({
        queryKey: RISK_KEYS.dashboard(variables.updates.organization_id),
      });
...
export function useCreateMitigation() {
  ...
  return useMutation({
    mutationFn: (mitigation: MitigationCreate) =>
      riskService.createMitigation(mitigation),
    onSuccess: (newMitigation, variables) => {
      // Invalidar queries relevantes
      queryClient.invalidateQueries({
        queryKey: RISK_KEYS.mitigations(variables.risk_id),
      });
      queryClient.invalidateQueries({ queryKey: ["risks", "mitigations"] });
...
export function useUpdateMitigation() {
  ...
  return useMutation({
    mutationFn: ({
      ...
      id: string;
      updates: Partial<MitigationCreate>;
    }) => riskService.updateMitigation(id, updates),
    onSuccess: (updatedMitigation, variables) => {
      // Invalidar queries relevantes
      queryClient.invalidateQueries({
        queryKey: RISK_KEYS.mitigations(updatedMitigation.risk_id),
      });
      queryClient.invalidateQueries({ queryKey: ["risks", "mitigations"] });
...
export function useMitigationMutations() {
  ...
  return {
    ...
    deleteMitigation,
    isLoading:
      createMitigation.isPending ||
      updateMitigation.isPending ||
      updateMitigationStatus.isPending ||
      deleteMitigation.isPending,
  };
}
...
export function useInvalidateMitigations() {
  ...
  return {
    byRisk: (riskId: string) => {
      queryClient.invalidateQueries({
        queryKey: ["risks", "mitigations", riskId],
      });
    },
    byOrganization: (organizationId: string) => {
      queryClient.invalidateQueries({
        queryKey: ["risks", "mitigations", "orgId", organizationId],
      });
    },
    all: () => {
      queryClient.invalidateQueries({ queryKey: ["risks", "mitigations"] });
