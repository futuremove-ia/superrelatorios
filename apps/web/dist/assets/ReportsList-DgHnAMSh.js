...
import { d as W, r, L as w } from "./router-XBfdTQ0K.js";
import { B as R } from "./badge-DD2chybY.js";
import { r as X } from "./mockReports-3cW05Ka2.js";
import {
  ak as Y,
  ag as T,
  S as E,
...
const ie = () => {
  ...
  (r.useEffect(() => {
    (async () => {
      try {
        const a = await X.getAllReports();
        (B(a), v(a));
      } catch (a) {
        console.error("Error loading reports:", a);
      } finally {
        V(!1);
      }
    })();
      ...
      (l &&
        (s = s.filter(
          (a) =>
            ...
            a.category.toLowerCase().includes(l.toLowerCase()),
        )),
        o !== "all" && (s = s.filter((a) => a.status === o)),
        n !== "all" && (s = s.filter((a) => a.category === n)),
        v(s));
    }, [j, l, o, n]));
  const h = (s) => {
      switch (s) {
    ...
    J = Array.from(new Set(j.map((s) => s.category)));
  return I
    ? e.jsx("div", {
                      ...
                      children: p.map((s, a) =>
                        e.jsxs(
                          u,
                          {
                            ...
                            style: { animationDelay: `${0.3 + a * 0.1}s` },
