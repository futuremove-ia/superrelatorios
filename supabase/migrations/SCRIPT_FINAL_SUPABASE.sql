-- =====================================================
-- SCRIPT FINAL CORRIGIDO - PAINEL DE INDICADORES SUPERRELATÓRIOS
-- Versão compatível com PostgreSQL
-- =====================================================

-- =====================================================
-- PARTE 1: CRIAR TIPOS (sem IF NOT EXISTS)
-- =====================================================

-- Apagar tipos se existirem para recriar
DO $$
BEGIN
...
-- Tabela cross-domain simplificada
...
CREATE POLICY IF NOT EXISTS "Public read metrics" ON metrics_library FOR SELECT USING (true);
CREATE POLICY IF NOT EXISTS "Service manage metrics" ON metrics_library FOR ALL USING (auth.role() = 'service_role');

CREATE POLICY IF NOT EXISTS "Users view org metrics" ON metric_values FOR SELECT USING (
    organization_id IN (SELECT organization_id FROM profiles WHERE id = auth.uid())
);
CREATE POLICY IF NOT EXISTS "Service manage values" ON metric_values FOR ALL USING (auth.role() = 'service_role');

CREATE POLICY IF NOT EXISTS "Public read analytics" ON cross_domain_analytics FOR SELECT USING (true);
CREATE POLICY IF NOT EXISTS "Service manage analytics" ON cross_domain_analytics FOR ALL USING (auth.role() = 'service_role');
...
SELECT 
    '✅ SCRIPT FINAL EXECUTADO COM SUCESSO!' as mensagem,
