-- =====================================================
-- SEED COMPLETO - SUPERRELATÓRIOS
-- Popula bibliotecas + organização demo + dados de exemplo
-- Executar no Supabase SQL Editor
-- =====================================================

-- =====================================================
-- 1. BIBLIOTECA DE KPIs (Por domínio)
-- =====================================================
...
('NOVOS_CLIENTES', 'Novos Clientes', 'New Customers', 'Quantidade de novos clientes no período', 'COUNT(DISTINCT cliente WHERE primeira_compra >= periodo_inicio)', 'unidade', 'commercial', 50, 35, 20, 10, 5, 'higher_is_better', 'non_cumulative', true),
...
-- =====================================================
-- 2. BIBLIOTECA DE CHALLENGES (Desafios comuns)
-- =====================================================
...
-- =====================================================
-- 3. BIBLIOTECA DE LEVERS (Alavancas estratégicas)
-- =====================================================
...
SELECT 
  ...
  'São Paulo',
  'SP',
  'contato@techpme.com.br',
  'pro',
  true
WHERE NOT EXISTS (SELECT 1 FROM organizations WHERE id = '00000000-0000-0000-0000-000000000001');

-- =====================================================
-- 6. PERFIL DEMO (usuário admin)
-- =====================================================

-- Nota: O profile é criado automaticamente pelo trigger on_auth_user_created
-- Este insert é apenas para demonstração se o trigger não funcionar

INSERT INTO profiles (id, user_id, organization_id, name, email, role)
SELECT 
  '00000000-0000-0000-0000-000000000001',
  '00000000-0000-0000-0000-000000000001',
  '00000000-0000-0000-0000-000000000001',
  'Admin Demo',
  'admin@techpme.com.br',
  'owner'
WHERE NOT EXISTS (SELECT 1 FROM profiles WHERE id = '00000000-0000-0000-0000-000000000001');

-- =====================================================
-- 7. REPORTS DE EXEMPLO
-- =====================================================

INSERT INTO reports (id, organization_id, user_id, title, description, status, data_json, created_at, updated_at) VALUES
('00000000-0000-0000-0000-000000000010', '00000000-0000-0000-0000-000000000001', '00000000-0000-0000-0000-000000000001', 'Relatório Executivo Q1 2026', 'Análise trimestral de performance com KPIs de todos os domínios', 'completed', 
  '{"category": "Executivo", "templateId": "executive-strategic", "blocks": [{"id": "b1", "type": "KPI_GRID", "layout": {"w": 12}, "content": {"title": "Principais Indicadores", "settings": {"columns": 3}, "data": {"Receita": "R$ 85.000", "Margem": "18%", "Novos Clientes": "42"}}}]}'::jsonb,
  '2026-03-01 10:00:00+00', '2026-03-15 14:30:00+00'),
('00000000-0000-0000-0000-000000000011', '00000000-0000-0000-0000-000000000001', '00000000-0000-0000-0000-000000000001', 'Análise Comercial Fevereiro', 'Performance de vendas e conversão do mês', 'completed',
  '{"category": "Comercial", "templateId": "sales-monthly", "blocks": []}'::jsonb,
  '2026-02-28 09:00:00+00', '2026-03-01 11:00:00+00'),
('00000000-0000-0000-0000-000000000012', '00000000-0000-0000-0000-000000000001', '00000000-0000-0000-0000-000000000001', 'Relatório Financeiro Março', 'Fluxo de caixa, receitas e despesas', 'draft',
  '{"category": "Financeiro", "templateId": "financial-monthly", "blocks": []}'::jsonb,
  '2026-03-20 08:00:00+00', '2026-03-20 08:00:00+00');

-- =====================================================
-- 8. RADAR ITEMS (Riscos e Oportunidades)
-- =====================================================

INSERT INTO radar_items (id, organization_id, type, title, diagnosis_code, impact_code, timeframe_code, complexity_code, severity, priority_score, domain, status, detected_at, ai_confidence_score, custom_notes) VALUES
-- Riscos
('00000000-0000-0000-0000-000000000020', '00000000-0000-0000-0000-000000000001', 'risk', 'Churn de Clientes Premium', 'DIAG_CHURN_RETENCAO', 'IMPACTO_RECEITA', 'TF_30DIAS', 'CX_BAIXA', 'high', 8, 'commercial', 'detected', NOW() - INTERVAL '3 days', 0.85, '5 clientes premium não renovaram nos últimos 30 dias'),
('00000000-0000-0000-0000-000000000021', '00000000-0000-0000-0000-000000000001', 'risk', 'Margem Comprimida por Custos', 'DIAG_MARGEM_CUSTOS', 'IMPACTO_LUCRO', 'TF_60DIAS', 'CX_MEDIA', 'high', 9, 'finance', 'detected', NOW() - INTERVAL '5 days', 0.92, 'Custos operacionais subiram 15% sem aumento proporcional de receita'),
('00000000-0000-0000-0000-000000000022', '00000000-0000-0000-0000-000000000001', 'risk', 'Inadimplência em Alta', 'DIAG_INADIMPLENCIA_COBRANCA', 'IMPACTO_CAIXA', 'TF_15DIAS', 'CX_BAIXA', 'critical', 10, 'finance', 'acknowledged', NOW() - INTERVAL '7 days', 0.95, 'Inadimplência atingiu 8.5%, acima do limite de 5%'),
('00000000-0000-0000-0000-000000000023', '00000000-0000-0000-0000-000000000001', 'risk', 'Rotatividade de Desenvolvedores', 'DIAG_CLIMA_ORGANIZACIONAL', 'IMPACTO_PRODUTIVIDADE', 'TF_90DIAS', 'CX_ALTA', 'medium', 6, 'people', 'detected', NOW() - INTERVAL '10 days', 0.78, '2 desenvolvedores seniores saíram no último trimestre'),

-- Oportunidades
('00000000-0000-0000-0000-000000000030', '00000000-0000-0000-0000-000000000001', 'opportunity', 'Upsell para Base Existente', null, null, 'TF_30DIAS', 'CX_BAIXA', 'medium', 7, 'commercial', 'detected', NOW() - INTERVAL '2 days', 0.82, '60% dos clientes usam apenas 1 serviço - potencial para cross-sell'),
('00000000-0000-0000-0000-000000000031', '00000000-0000-0000-0000-000000000001', 'opportunity', 'Otimização de Preços', null, null, 'TF_60DIAS', 'CX_MEDIA', 'high', 8, 'finance', 'acknowledged', NOW() - INTERVAL '4 days', 0.88, 'Análise mostra que preços estão 12% abaixo da concorrência'),
('00000000-0000-0000-0000-000000000032', '00000000-0000-0000-0000-000000000001', 'opportunity', 'Automação de Marketing', null, null, 'TF_90DIAS', 'CX_MEDIA', 'medium', 6, 'marketing', 'detected', NOW() - INTERVAL '1 day', 0.75, 'Implementar fluxos de nutrição pode aumentar conversão em 25%');

-- =====================================================
-- 9. ACTION ITEMS (Ações para resolver radar items)
-- =====================================================
...
('00000000-0000-0000-0000-000000000040', '00000000-0000-0000-0000-000000000001', '00000000-0000-0000-0000-000000000020', 'Criar programa de fidelidade para clientes premium', 'in_progress', 1, 'Desenvolver programa com benefícios exclusivos e descontos progressivos', NOW() - INTERVAL '2 days', NOW() - INTERVAL '1 day', NOW() + INTERVAL '30 days'),
('00000000-0000-0000-0000-000000000041', '00000000-0000-0000-0000-000000000020', 'Implementar pesquisa de satisfação NPS', 'pending', 2, 'Enviar pesquisa para todos os clientes ativos', NOW() - INTERVAL '1 day', NOW() - INTERVAL '1 day', NOW() + INTERVAL '14 days'),
...
-- =====================================================
-- 10. ORGANIZATION KPIs (Valores reais para demo)
-- =====================================================
...
-- =====================================================
-- SUCESSO! Seed completo executado.
-- =====================================================
