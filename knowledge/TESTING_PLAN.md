...
test.describe('Report Generation Flow', () => {
  test('complete report generation workflow', async ({ page }) => {
    ...
    await page.fill('[data-testid="report-title"]', 'Relatório de Vendas Q3 2024');
    await page.selectOption('[data-testid="report-template"]', 'sales');
    await page.fill('[data-testid="report-description"]', 'Análise completa das vendas do terceiro trimestre');
...
test.describe('Internationalization Tests', () => {
  ...
  test('date and number formatting by locale', async ({ page }) => {
    ...
    for (const { lang, expectedDate, expectedNumber, expectedCurrency } of formatTests) {
      ...
      const numberElement = await page.locator('[data-testid="formatted-number"]');
      const currencyElement = await page.locator('[data-testid="formatted-currency"]');
