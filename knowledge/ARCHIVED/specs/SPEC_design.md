...
### Progressive Disclosure — Revelação Progressiva de Complexidade

O sistema tem 23 tipos de objetos de UI. Expô-los simultaneamente causaria sobrecarga cognitiva para o gestor de PME. A solução é revelar complexidade em 4 camadas progressivas.

**Modelo mental do usuário (4 conceitos, não 23):**
...
| ------------ | --------------------------------------------------- | ------------------------ |
| 1 — Núcleo   | `KPICard`, `RadarCard`, `ActionCard`                | Sempre, desde o Dia 1    |
| 2 — Contexto | Drawer com diagnóstico, impacto, alavancas          | Ao clicar em um objeto   |
| 3 — Módulos  | Riscos, Fornecedores, Clientes, RH, Previsões, SWOT | Por setor/maturidade     |
| 4 — Config   | Blueprint, Hierarquia, Data Sources                 | Setup inicial, raramente |

**Modos de operação** (controlado por `organization_ui_preferences.modo_operacao`):

- `simples` — apenas Camada 1. Para PME early-stage, primeiro mês.
- `completo` — Camada 1 + módulos setoriais relevantes. Padrão após 30 dias.
- `enterprise` — tudo, incluindo hierarquia e analytics avançados.

**Ativação automática de módulos por setor:**

- Varejo / Indústria / Alimentação → módulo Fornecedores
- B2C / Marketplace → módulo Saúde de Clientes
- `employee_count_range` > '11-50' → módulo Custos de RH
- `blueprint.completeness_score` > 60% → SWOT
- 3+ meses de dados → Previsões de curto prazo

**Regras de UX:**

- Máximo 7 itens na sidebar. Excedente em seções colapsáveis.
- Nunca exibir card vazio — sempre estado vazio com CTA contextual.
- Módulos ocultos não geram notificações ou badges de "configure agora".
- `linguagem_preferencia = 'simples'` usa `ui_translation_glossary` para termos acessíveis.
...
```mermaid
graph LR
    KPIs["KPIs da Organização"] --> CALC["Serviços de Cálculo\nRUNWAY, LTV_CAC, CHURN..."]
    CALC --> DETECT["DetectionService\nCASH_FLOW_CRUNCH\nINEFFICIENT_SALES_MACHINE\nCOMMODITY_TRAP"]
...
// NOTA: O serviço MetricsCalculationService não existe no código real
// Verificar implementações existentes em:
...
// Exemplo com serviço existente:
// import { KPICalculationEngine } from "../domain/metrics/engine/KPICalculationEngine";
// const calculationService = new KPICalculationEngine(...);
