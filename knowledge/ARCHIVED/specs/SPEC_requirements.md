...
### Requisito 42: Progressive Disclosure e Redução de Carga Cognitiva

**User Story:** Como gestor de PME sem experiência em ferramentas de analytics, quero que a plataforma me mostre apenas o que é relevante para o meu momento atual, para que eu não me sinta sobrecarregado e consiga extrair valor desde o primeiro dia.

#### Critérios de Aceitação

1. THE Sistema SHALL exibir no máximo 3 objetos primários na tela inicial para novos usuários: `KPICard`, `RadarCard` e `ActionCard` — correspondendo às perguntas "Como estou?", "O que está errado?" e "O que fazer?".

2. THE Sistema SHALL revelar informações detalhadas (diagnóstico, impacto, alavancas) apenas quando o usuário clicar em um objeto — nunca na listagem inicial.

3. THE Sistema SHALL suportar três modos de operação via `organization_ui_preferences.modo_operacao`:
   - `simples`: apenas Camada 1 (KPIs, Radar, Ações, Relatórios básicos)
   - `completo`: Camada 1 + módulos setoriais relevantes + riscos + previsões
   - `enterprise`: tudo, incluindo hierarquia organizacional e analytics avançados

4. WHEN um novo usuário completa o onboarding, THE Sistema SHALL ativar `modo_operacao = 'simples'` por padrão, revelando apenas os 3 objetos primários.

5. WHEN o usuário completa 30 dias de uso com dados reais, THE Sistema SHALL sugerir upgrade para `modo_operacao = 'completo'` com explicação dos módulos que serão desbloqueados.

6. THE Sistema SHALL ativar módulos setoriais automaticamente com base no `industry_sector` da organização:
   - Varejo, Indústria, Alimentação → ativar módulo de Fornecedores
   - B2C, Marketplace → ativar módulo de Saúde de Clientes
   - Empresas com `employee_count_range` > '11-50' → ativar módulo de Custos de RH

7. THE Sistema SHALL exibir no máximo 7 itens na sidebar de navegação. Quando mais módulos estiverem ativos, agrupá-los em seções colapsáveis.

8. WHEN `linguagem_preferencia = 'simples'`, THE Sistema SHALL substituir termos técnicos por equivalentes acessíveis via `ui_translation_glossary` (ex: "Margem de Contribuição" → "O que sobra de cada venda").

9. THE Sistema SHALL nunca exibir um card vazio sem dados — sempre mostrar estado vazio com CTA contextual (ex: "Adicione seu primeiro KPI" ou "Importe um relatório").

10. WHEN `blueprint.completeness_score < 60%`, THE Sistema SHALL exibir aviso contextual apenas nas telas de análise — nunca como notificação intrusiva ou bloqueio de acesso.

11. FOR ALL módulos ocultos, THE Sistema SHALL garantir que sua ausência não gere notificações, badges ou indicadores de "configure agora" — módulos aparecem quando o usuário está pronto, não antes.
