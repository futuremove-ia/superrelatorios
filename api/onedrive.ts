...
async function saveCloudConfig(
  ...
  if (!response.ok) {
    ...
    console.error("[onedrive-oauth] Failed to save config:", error);
  ...
  if (req.method === "GET") {
    ...
    if (action === "auth-url") {
      if (!ONEDRIVE_CLIENT_ID || !ONEDRIVE_REDIRECT_URI) {
        return jsonResponse({ error: "OneDrive OAuth not configured" }, 503);
      }

      const scope = encodeURIComponent(
        "Files.Read.All Sites.Read.All offline_access",
      );
      const authUrl = `https://login.microsoftonline.com/common/oauth2/v2.0/authorize?client_id=${ONEDRIVE_CLIENT_ID}&redirect_uri=${encodeURIComponent(ONEDRIVE_REDIRECT_URI)}&response_type=code&scope=${scope}&state=${orgId}`;

      return jsonResponse({ authUrl }, 200);
    }

    if (action === "status") {
      const config = await getCloudConfig(orgId, "onedrive");
  ...
  if (req.method === "POST") {
    ...
    if (action === "connect") {
      ...
      try {
        ...
        await saveCloudConfig(orgId, "onedrive", credentials);

        return jsonResponse({ success: true }, 200);
      } catch (err) {
        console.error("[onedrive-oauth] Connect error:", err);
        return jsonResponse({ error: "Failed to connect" }, 500);
      }
    }

    if (action === "disconnect") {
      await deleteCloudConfig(orgId, "onedrive");
      return jsonResponse({ success: true }, 200);
    }
