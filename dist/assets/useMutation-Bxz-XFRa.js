...
import { r as f } from "./vendor-Bp-AcFIh.js";
import {
  U,
  W as k,
  X as w,
...
function T(i, t) {
  ...
  const n = f.useSyncExternalStore(
      f.useCallback((c) => r.subscribe(R.batchCalls(c)), [r]),
      () => r.getCurrentResult(),
      () => r.getCurrentResult(),
    ),
    v = f.useCallback(
      (c, y) => {
        r.mutate(c, y).catch(j);
      },
      [r],
    );
